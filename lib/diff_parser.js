...
DiffParser.prototype._chunkHeader = function(line) {
	...
	parts = linenums.split(" ");
	parts.forEach(function(chunk) {
		if (!chunk.length)
			return;
		var pieces = chunk.split(",");
		var sign = pieces[0].charAt(0);
		var count = 0;
		var linenum = Math.abs(parseInt(pieces[0]));
		if (pieces.length > 1) {
		...
		self._chunks.push([linenum, count]);
	});
}
...
DiffParser.prototype._fromFile = function(line) {
	...
	if (self._fromFilename != "/dev/null") {
		// strip off the "a/"
		self._fromFilename = self._fromFilename.slice(self._fromFilename.indexOf('/')); 
	}
}
...
DiffParser.prototype._toFile = function(line) {
	...
	if (self._toFilename != "/dev/null") {
		// strip off the "a/"
		self._toFilename = self._toFilename.slice(self._toFilename.indexOf('/')); 
	}
}
