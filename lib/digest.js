...
Digest.prototype.buildBranchInfo2 = function(branch_name, start, max) {
	...
	.then(function() {
		...
		var bp = new BranchProcessor(self._repoName, branch_name);
...
var BranchProcessor = function(repo, branch) {
	var self = this;
	self._count = 0;
	self._commitIds = [];
	self._repoName = repo;
...
BranchProcessor.prototype.persistCommit = function(commit) {
	...
	Logger.DEBUG("Diff summary", 
		JSON.stringify(commit.diff.diffSummary()),
		'\n', 
		Logger.CHANNEL.DIGEST);
