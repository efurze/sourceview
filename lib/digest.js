...
Digest.prototype.buildBranchInfo = function(branch_name, start, max) {
	...
	return self.saveRevList(branch_name, start, max)
...
Digest.prototype.saveRevList = function(branch_name, start, max) {
	var self = this;


	status("Building revision history for", branch_name);
	return self.getRevList(branch_name, max)
		.then(function(history) {
...
Digest.prototype.getRevList = function(branch_name, max) {
	var self = this;
...
BranchProcessor.prototype.persistCommit = function(commit) {
	...
	return Persist.saveCommit(self._repoName, 
		...
		if (count % 100 == 0)
			status("Saving diff", count, commit.info.id);
